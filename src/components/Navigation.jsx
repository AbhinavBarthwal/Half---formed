      ...
      <header className="w-full flex items-center justify-between px-4 sm:px-8 py-3.5 border-b border-black/10 dark:border-white/10 glass-4 z-40 sticky top-0 transition-colors">
        ...
        >
          <div className="relative w-6 h-6">
            <div className="absolute inset-0 rounded-full border border-crimson-700 dark:border-navy-300 border-dashed animate-spin-slow" />
          ...
          <span className="font-serif text-xl tracking-wide text-black dark:text-white font-black group-hover:text-crimson-700 dark:group-hover:text-crimson-400 transition-colors">
            half-formed
          </span>
        </div>

        {/* Desktop Nav Links — Pure Apple HIG Segmented Control without hideous outer border */}
        <nav className="hidden md:flex items-center gap-1 bg-black/5 dark:bg-white/10 p-1 rounded-full backdrop-blur-md">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-mono font-black text-black dark:text-white px-3 py-1.5 rounded-full bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/20 transition-all"
          ...
          <button
            onClick={toggleTheme}
            className="p-2 rounded-full text-black dark:text-white bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/20 transition-all interactive-scale"
          ...
          >
            <Sparkles size={13} className="animate-pulse text-amber-400 dark:text-amber-600" /> Art Lab
          </button>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 glass-4 border-t border-black/10 dark:border-white/10 px-3 py-2 flex items-center justify-around z-40 pb-safe">
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-0.5 py-1 px-3 rounded-xl transition-all ${
                isActive ? 'text-black dark:text-white font-black scale-105' : 'text-black/60 dark:text-white/60 font-semibold'
              }`}
            >
              <Icon size={18} />
        ...
        {isAuthenticated && (
          <button
            onClick={() => onNavigate('start')}
            className={`flex flex-col items-center gap-0.5 py-1 px-3 rounded-xl transition-all ${
              currentView === 'start' ? 'text-black dark:text-white font-black scale-105' : 'text-black/60 dark:text-white/60 font-semibold'
            }`}
          >
            <Plus size={18} />
