  ...
  const slides = [
    {
      ...
      badgeColor: "text-sage-signal border-sage-signal/30 bg-sage-signal/10"
    },
    {
      icon: Shield,
      tag: "CORE PRINCIPLES",
      ...
      badgeColor: "text-philosophy-gold border-philosophy-gold/30 bg-philosophy-gold/10"
    },
    {
      icon: Users,
      ...
      badgeColor: "text-dusk-lavender border-dusk-lavender/30 bg-dusk-lavender/10"
    }
  ];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/70 backdrop-blur-2xl">
      <motion.div
        ...
        className="w-full max-w-lg glass-panel p-6 sm:p-8 rounded-3xl shadow-2xl relative overflow-hidden flex flex-col justify-between min-h-[440px]"
      >
        {/* Top bar */}
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-full bg-sage-signal animate-pulse" />
            <span className="font-serif text-sm tracking-wider text-parchment font-semibold">half-formed</span>
          </div>
          <button
            onClick={onDismiss}
            className="text-ash hover:text-parchment p-1.5 rounded-full hover:bg-white/10 transition-colors"
            title="Skip Intro"
          >
            <X size={18} />
          </button>
        ...
        <div className="my-auto py-6">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <h2 className="font-serif text-2xl sm:text-3xl text-parchment leading-tight">
                {current.title}
              </h2>
            ...
            {slides.map((_, idx) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all ${
                  slide === idx ? 'w-6 bg-sage-signal' : 'w-2 bg-white/20 hover:bg-white/40'
                }`}
              />
            ))}
          </div>
          ...
          <div className="flex items-center gap-3">
            {slide > 0 && (
              <button
                ...
                className="px-4 py-2 text-xs font-mono text-ash hover:text-parchment transition-colors"
              >
                Back
              </button>
            )}
            ...
            <button
              onClick={handleNext}
              className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-parchment text-ink-deep font-semibold text-xs hover:bg-white transition-all shadow-lg"
            >
